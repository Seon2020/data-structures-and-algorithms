...
const templatingWithMustache = () => {
  ...
  characters.forEach(characters => {
    let display = Mustache.render($template, characters);
    output.push(display)
  });
  return output;
}
...
const hasChildrenValues = (arr, character) => {
  for (let pplVal in arr) {
    if (Object.values(arr[pplVal].children) && arr[pplVal].name == character) {
      return true;
    }
  }
};
